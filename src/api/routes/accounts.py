...
async def register(
    data: UserRegisterRequest, db: AsyncSession = Depends(get_db)
) -> UserResponse:
...
async def login(
    data: UserLoginRequest, db: AsyncSession = Depends(get_db)
) -> TokenPairResponse:
...
async def refresh_token(data: RefreshTokenRequest, db: AsyncSession = Depends(get_db)):
    tokens = await refresh_access_token(db=db, refresh_token=data.refresh_token)
...
async def logout(data: RefreshTokenRequest, db: AsyncSession = Depends(get_db)) -> None:
    await logout_user(db=db, refresh_token=data.refresh_token)
...
@router.patch("/admin/users/{user_id}/group", response_model=UserResponse)
async def admin_change_user_group_endpoint(
    ...
    user = await change_user_group(db=db, user_id=user_id, group=data.group)
...
async def change_password_endpoint(
    ...
    await change_password(
        db=db, user=user, old_password=data.old_password, new_password=data.new_password
    )
...
async def update_profile_endpoint(
    ...
    profile = await update_user_profile(db=db, user_id=user.id, data=data)
...
async def upload_avatar_endpoint(
    ...
    profile = await update_user_avatar(db=db, user_id=user.id, file=file)
